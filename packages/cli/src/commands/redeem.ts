...
export default class Redeem extends BaseCommand {
  static summary = "Redeem Stablecoin for collateral.";

  static description = `
This command burns Stablecoin from your wallet and withdraws the underlying collateral from the selected vault.
`;
  ...
  async run(): Promise<void> {
    const { flags } = await this.parse(Redeem);
    ...
    this.logger.info("Redeeming Jupiter Stable LP tokens with:");
    ...
    try {
      ...
      console.error(error);
      this.logger.error(`Redeem failed: ${(error as Error).message}`);
      process.exit(1);
    }
  }
}
